function ConditionAveragingScriptWF(info,miceopt,StorePath,DataDirectory,Stim2Check,baselinemethod,trialtype,timelim,smoothfact,takeequalsample,timebins,Redo)
...
%                                 end
                        ...
                        for i = 1:100:size(tracesthiscond,1) %Can't do the whole dataset in one go, internal conversion to double?
                            ...
                            if strcmp(TRIALTYPE,'1500')
                            AverageBins{stidx,ridx}(i:i+99,:,:) = nanmean(tmp(:,:,timebins(1),:),3); %average over trials
                ...
                %                 end
                %Save data
                ChoiceProbabilities.dFFav = dFFav;
                ChoiceProbabilities.nrt = nrt;
                ChoiceProbabilities.meanRT = meanRT;
                ChoiceProbabilities.stdRT = stdRT;
                ChoiceProbabilities.ReactionOpt = ReactionOpt;
                ChoiceProbabilities.SideOpt = SideOpt;
                ChoiceProbabilities.ConditionNames = ConditionNames;
                ChoiceProbabilities.SUMSqr = SUMSqr;
                ChoiceProbabilities.SumdFF = SumdFF;
                ChoiceProbabilities.nrtPerPix = nrtPerPix;
                
                save(fullfile(StorePath,mouse,[mouse date],[mouse num2str(expnr)],['Baseline' num2str(baselinemethod) '_' TRIALTYPE ,'_eqsample' num2str(takeequalsample)],'ChoiceProbabilities'),'ChoiceProbabilities','-v7.3')
