...
%     specify the maximum number of workers in the parallel pool that will
                     ...
                     parfor (ridx = 1:length(ReactionOpt),4)
                            leftdat = [];
                            rightdat = [];

                        leftidx = find(~cellfun(@isempty,(cellfun(@(X) strfind(X,ReactionOpt{ridx}),ConditionNamestmp,'UniformOutput',0)))& ~cellfun(@isempty,(cellfun(@(X) strfind(X,'Left'),ConditionNamestmp,'UniformOutput',0))));
                        if strcmp(Stim2Check,'DelayedOriTuningSound')
...
%                             clear tmpload
                        ...
                        rightidx = find(~cellfun(@isempty,(cellfun(@(X) strfind(X,ReactionOpt{ridx}),ConditionNamestmp,'UniformOutput',0)))& ~cellfun(@isempty,(cellfun(@(X) strfind(X,'Right'),ConditionNamestmp,'UniformOutput',0))));
                        if strcmp(Stim2Check,'DelayedOriTuningSound')
                ...
                if wholebrainana
                    thistimer = tic;
                    tmphit = reshape(leftdat,[xpix*ypix,nright]);
                    tmperror = reshape(rightdat,[xpix*ypix,nleft]);
                    tmperror = tmperror';
                    tmphit = tmphit';
