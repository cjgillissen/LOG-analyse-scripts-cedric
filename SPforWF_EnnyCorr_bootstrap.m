...
% EvokedActivityROI = 1;
wholebrainana = 2; % if 2 then per area plus bootstrap confidence bounds
newsize = [400 400];
scalefct = 0.5;
cpimrange = [0.3 0.7];
            ...
            elseif wholebrainana ==2
                 ...
                 for roiidx = 1:length(regio2take)
                    ...
                    for roi2dx = 1:length(Borders)
                        ...
                        count = count+1;
                        leftdat(~repmat(mask,[1,1,size(leftdat,3)]))=nan;
                        leftdat = nanmean(reshape(leftdat,[xpix*ypix,size(leftdat,3)]),1);

                        rightdat(~repmat(mask,[1,1,size(rightdat,3)]))=nan;
                        rightdat = nanmean(reshape(rightdat,[xpix*ypix,size(rightdat,3)]),1);
                        ...
                        L2 = size(leftdat,2);
                        labels = [ones(L1,1);zeros(L2,1)];
                        scores = [rightdat;leftdat];
                        [~,~,~,AUC1] = perfcurve(labels,scores,1,'Nboot',1000);
                        AUC  = AUC(1);
                        tmplowerbound = AUC1(2);
                        tmpupperbound = AUC1(3);
                        
                        Perf{midx,twid,count}.SP = AUC;
                        Perf{midx,twid,count}.nrright = L1;
                        Perf{midx,twid,count}.nrleft = L2;
                        Perf{midx,twid,count}.lowerbound = tmplowerbound;
                        Perf{midx,twid,count}.upperbound = tmpupperbound;
                        Perf{midx,twidx,count}.rname = BrainModel{midx}.Model.Rnames{regio2take(roiidx)};
                        disp(['SP analysis ' num2str(TW{twid}(1)) '-' num2str(TW{twid}(2)) ', ' trialtypes{id} ' took ' num2str(toc(thistimer)./60) ' minutes'])
                        saveas(Hwhole,fullfile('C:\Users\gillissen\Desktop\Figures SP',['SP LeftvsRight' num2str(TW{twid}(1)) '-' num2str(TW{twid}(2)) trialtypes{id} mouse]))
               end
            end
