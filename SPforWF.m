function SPforWF(info,miceopt,StorePath,Stim2Check,baselinemethod,trialtypes,ReactOpt,takeequalsample)
...
for midx = 1:nrMouse %For this mouse
    ...
    for ridx = 1:length(ReactOpt)
        Hwhole = figure('units','normalized','outerposition',[0 0 1 1],'name',[mouse '_Allsess_'  ReactOpt{ridx} ' wholebrain']);
        Hspot = figure('units','normalized','outerposition',[0 0 1 1],'name',[mouse '_Allsess_'  ReactOpt{ridx} ' Spotlight']);
            ...
            for id = 1:length(trialtypes)
                ...
                for didx = 1:size(logs,2) %Loop over days
                    ...
                    for sidx = 1:size(logs,3) %If no xth session, continue
                        ...
                        % load in rawdata
                        rightidx = find(~cellfun(@isempty,(cellfun(@(X) strfind(X,'Right'),ConditionNamestmp,'UniformOutput',0)))& ~cellfun(@isempty,(cellfun(@(X) strfind(X,ReactOpt{ridx}),ConditionNamestmp,'UniformOutput',0))));
                        if strcmp(Stim2Check,'DelayedOriTuningSound')
                        ...
                        leftidx = find(~cellfun(@isempty,(cellfun(@(X) strfind(X,'Left'),ConditionNamestmp,'UniformOutput',0))) & ~cellfun(@isempty,(cellfun(@(X) strfind(X,ReactOpt{ridx}),ConditionNamestmp,'UniformOutput',0))));
                        if strcmp(Stim2Check,'DelayedOriTuningSound')
                ...
                if wholebrainana
                    ...
                    tmpsp = zeros(size(tmpleft,2),1);
                 
                    %% compute CP for whole brain
                    % how to get back to brainpixelssss???
                   for pixidx = 1:size(tmpleft,2)
                       
                    L1 = size(tmpleft(:,pixidx),1);
                    L2 = size(tmpright(:,pixidx),1);
                   ...
                   newauc(aucform) = smooth2a(tmpsp,2,2);
                   ...
                   title([num2str(TW{twid}(1)) '-' num2str(TW{twid}(2)) ', ' trialtypes{id}, 'Stimulus Sensitivity per Pixel'])
                    ...
                    Perf{midx,sessioncount,ridx,id,twid}.SP = newauc;
...
save(fullfile('C:\Users\gillissen\Desktop\Figures CP',['Performance SP' strjoin(trialtypes) strjoin(miceopt)]),'Perf')
